...
iFrame <- sys.nframe()
if (iFrame >= 3) { script.dir.mapearEx <- sys.frame(iFrame - 3)$ofile 
} else { script.dir.mapearEx <- NULL }
while ((is.null(script.dir.mapearEx) || is.na(regexpr('mapearEx.r', script.dir.mapearEx, fixed=T)[1])) && iFrame >= 0) {
  script.dir.mapearEx <- sys.frame(iFrame)$ofile
  iFrame <- iFrame - 1
}
