  ...
  if (!mapaConstante) {
    ...
    if (params$umbralMascaraCeros > 0 & abs(min(observaciones$value)) < 1E-3) {
      interpolacion <- aplicarMascaraRnR(
        observaciones=observaciones, interpolacion=interpolacion, params=params, 
        shpMask=shpMask
      )
...
cachearRegresoresEstaticos <- function(
    coordsObservaciones, coordsAInterpolar, nCoresAUsar=0, recalculateIfAlreadyExists=FALSE
) {
  ...
  if (
    ...
    file.info(pathCacheDatosCoordsObservaciones)$size <= 0
  ) {
    dfDatosCoordsObservaciones <- list()
...
incorporarRegresoresEstaticos <- function(
  ...
  if (
    ...
    cachearRegresoresEstaticos(
      coordsObservaciones=geomCoordsObs,
      coordsAInterpolar=geomCoordsInterp,
      nCoresAUsar=params$nCoresAUsar,
      recalculateIfAlreadyExists=TRUE
    )
...
universalGriddingCV_i <- function(
    iObservacion=1, tIni=1, tFin=nrow(valoresObservaciones), coordsObservaciones, 
    fechasObservaciones, valoresObservaciones, params, 
    valoresRegresoresSobreObservaciones=NULL, longitudesEnColumnas=T, 
...
universalGriddingCV <- function(
  ...
  if (params$nCoresAUsar <= 0) { 
    nCoresAUsar <- min(getAvailableCores(maxCoresPerGB=1), length(iesAEstimar))
  } else { nCoresAUsar <- params$nCoresAUsar }
  ...
  if (length(params$tlagsAR) <= 0) {
    if (nCoresAUsar > 1) {
      ...
      # iesAEstimar <- c(195, 196)
      #iesAEstimar <- seq.int(150, 196) # funco
      #iesAEstimar <- seq.int(100, 125) # mal
      #iesAEstimar <- seq.int(100, 112) # funco
      #iesAEstimar <- seq.int(113, 118) # mal
